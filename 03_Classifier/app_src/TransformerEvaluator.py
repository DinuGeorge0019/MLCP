...
from app_src.SentenceTransformerWrapper import SentenceTransformerWrapper
...
class TransformerEvaluator():
    ...
    def __save_metrics(self, encoder, estimator_name, metrics_results, number_of_tags):
        ...
        if not os.path.isfile(CONFIG[f'TOP_{number_of_tags}_BENCHMARK_TRANSFORMER_MODELS_PATH']):
            # Write the DataFrame to the csv file
            df.to_csv(CONFIG[f'TOP_{number_of_tags}_BENCHMARK_TRANSFORMER_MODELS_PATH'], index=False)
        else:
